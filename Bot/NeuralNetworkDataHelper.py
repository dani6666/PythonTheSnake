...
class NeuralNetworkDataHelper:
    @staticmethod
    def generate_data():
        return NeuralNetworkData(numpy.random.rand(16, 12), numpy.random.rand(12, 12), numpy.random.rand(12, 4))

    @staticmethod
    def crossover_data(first_data, second_data):
        result = NeuralNetworkData(numpy.zeros(16, 12), numpy.zeros(12, 12), numpy.zeros(12, 4))
        for i in range(16):
            cross_point = random.randint(-1, 13)
            for j in range(12):
                if j < cross_point:
    ...
    def mutate_data(data_to_mutate):
        ...
        for i in range(16):
            first_mutation = random.randint(0, 9)
        ...
        for i in range(12):
            first_mutation = random.randint(0, 2)
